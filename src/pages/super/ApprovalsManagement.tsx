...
export const ApprovalsManagement: React.FC = () => {
  ...
  const classFilteredList = currentTabList.filter((p) => {
  ...
  // Calculate stats for current class filter
  const filteredPendingPayments = pendingPayments.filter((p) =>
    selectedClassFilter === "all" ? true : p.classId === selectedClassFilter
  );
  ...
  const handleApproveSingle = async (p: PaymentModel) => {
    if (!userProfile) return;
    try {
      await approvePayment(p.id, userProfile);
      await loadData();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleBatchApproveConfirm = async () => {
    ...
    try {
      await batchApprovePayments(selectedList, userProfile);
      await loadData();
      setSelectedPaymentIds(new Set());
  ...
  const handleRollbackSubmit = async () => {
    if (!rollbackTarget || !userProfile) return;
    try {
      await rollbackPayment(rollbackTarget.id, rollbackReason, userProfile);
      await loadData();
      ...
      <div className="glass-panel rounded-3xl border border-slate-800 overflow-hidden">
        <div className="p-4 border-b border-slate-800/80 flex items-center justify-between">
          <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider">
            {filterTab.toUpperCase().replace("_", " ")} ENTRIES ({filteredList.length}) • Scope: {selectedClassFilter === "all" ? "All Classes" : classMap.get(selectedClassFilter)}
          </h3>
        </div>
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {p.status === "approved" && (
                            <button
                              ...
                              className="px-2.5 py-1 bg-amber-950 hover:bg-amber-900 text-amber-300 border border-amber-800 text-xs font-semibold rounded-lg transition-colors flex items-center gap-1 cursor-pointer"
                            >
