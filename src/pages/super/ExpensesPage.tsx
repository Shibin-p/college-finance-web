...
export const ExpensesPage: React.FC = () => {
  ...
  // Category-Wise Total Spending Summary
  const categorySummaryMap: Record<string, { total: number; count: number }> = {};
  activeConfiguredCategories.forEach((c) => {
    categorySummaryMap[c.name] = { total: 0, count: 0 };
  });
  activeExpenses.forEach((e) => {
    if (!categorySummaryMap[e.category]) {
      categorySummaryMap[e.category] = { total: 0, count: 0 };
    }
    categorySummaryMap[e.category].total += e.amount || 0;
    categorySummaryMap[e.category].count += 1;
  });

  const selectedCategorySpent =
    selectedCategory !== "all" ? categorySummaryMap[selectedCategory]?.total || 0 : totalActiveSpent;
  ...
  const handleExportSummaryExcel = async () => {
    ...
    setExporting(true);
    try {
      await exportExpensesSummaryExcel(activeEvent.name, expenses, userProfile);
    } catch (err) {
      console.error(err);
    } finally {
      setExporting(false);
  ...
  const handleExportSummaryPDF = async () => {
    ...
    setExporting(true);
    try {
      await exportExpensesSummaryPDF(activeEvent.name, expenses, userProfile);
    } catch (err) {
      console.error(err);
    } finally {
      setExporting(false);
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 p-1 rounded-xl">
            <button
              ...
              title="Export Detailed Excel (Includes Category Summary Sheet)"
              className="px-2.5 py-1.5 hover:bg-slate-800 text-emerald-400 font-bold text-xs rounded-lg transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={exporting || expenses.length === 0}
              title="Export Category-Wise Summary Excel"
              className="px-2 py-1.5 hover:bg-slate-800 text-slate-300 text-[11px] font-semibold rounded-lg transition-all cursor-pointer disabled:opacity-50 border-l border-slate-800"
            >
              Summary
            </button>
          </div>
          ...
          <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 p-1 rounded-xl">
            <button
              ...
              className="px-2.5 py-1.5 hover:bg-slate-800 text-slate-200 font-bold text-xs rounded-lg transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={exporting || expenses.length === 0}
              title="Export Category-Wise Summary PDF"
              className="px-2 py-1.5 hover:bg-slate-800 text-slate-300 text-[11px] font-semibold rounded-lg transition-all cursor-pointer disabled:opacity-50 border-l border-slate-800"
            >
              Summary
            </button>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/80 pb-3">
          <div>
            <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider flex items-center gap-2">
              ...
              <span>Category-Wise Spending Summary</span>
            </h3>
            <p className="text-xs text-slate-400 mt-0.5">
              Breakdown of all active expenditures across configured and custom categories. Click a card to filter.
            </p>
          </div>

          <div className="flex items-center gap-2 self-start sm:self-auto">
            {selectedCategory !== "all" && (
              <div className="px-3 py-1 rounded-xl bg-teal-500/10 border border-teal-500/30 text-xs flex items-center gap-1.5">
                <span className="text-slate-400">{selectedCategory}:</span>
                <span className="font-mono font-bold text-teal-300">{formatINR(selectedCategorySpent)}</span>
                <span className="text-[11px] font-mono text-teal-400">
                  ({totalActiveSpent > 0 ? ((selectedCategorySpent / totalActiveSpent) * 100).toFixed(1) : 0}%)
                </span>
              </div>
            )}
            <div className="px-3 py-1 rounded-xl bg-slate-900 border border-slate-800 text-xs flex items-center gap-1.5 font-mono">
              <span className="text-slate-400">Overall:</span>
              <span className="font-bold text-rose-400">{formatINR(totalActiveSpent)}</span>
            </div>
          </div>
        </div>

        {Object.keys(categorySummaryMap).length === 0 ? (
          <p className="text-xs text-slate-500 py-2 text-center">No categories configured yet.</p>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            {Object.entries(categorySummaryMap).map(([catName, data]) => {
              const isSelected = selectedCategory === catName;
              const percent = totalActiveSpent > 0 ? (data.total / totalActiveSpent) * 100 : 0;
              return (
                <button
                  key={catName}
                  type="button"
                ...
                >
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="font-bold text-slate-200 truncate pr-1">{catName}</span>
                    <span className="text-[10px] font-mono text-slate-400">{percent.toFixed(0)}%</span>
                  </div>
